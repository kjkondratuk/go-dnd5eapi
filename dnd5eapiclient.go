...
type (
	Client struct {
		...
		MagicSchools      magic_schools.MagicSchoolsClient
...
func NewClient(basicsProvider api.BasicsProvider) *Client {
	...
	client.ClassProficencies = classes_proficiency_rel.NewClient(client.Classes, client.Proficiencies)
	return client
}
