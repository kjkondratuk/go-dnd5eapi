...
func TestWeaponProperties_GetList_IT(t *testing.T) {
...
func TestWeaponProperties_QueryList_IT(t *testing.T) {
	query := make(map[string]string, 1)
	query["index"] = "ammunition"
...
func TestWeaponProperties_GetByIndex_IT(t *testing.T) {
	resp, err := Client.GetByIndex("ammunition")
	r, _ := json.Marshal(resp)
	log.Print(fmt.Sprintf("Response: %s", r))
	assert.Nil(t, err, "Should not receive an error contacting API.")
	assert.True(t, true, "Should complete successfully!")
